...
model LynxLQVelSimlpeParachute
  ...
  RocketControl.Rockets.Lynx.LynxSimpleAeroSimplePara lynxRocket annotation(
  ...
  inner World.Atmosphere atmosphere(wind_speed = {1, 1, 0})  annotation(
...
equation
  ...
  connect(lynxRocket.ref_center, sampledTrueSensors.frame_a) annotation(
    Line(points = {{0, 10}, {30, 10}, {30, -30}, {40, -30}}));
  ...
  connect(lynxRocket.bus, sampledTrueSensors.bus) annotation(
    Line(points = {{0, 20}, {80, 20}, {80, -30}, {60, -30}}, color = {255, 204, 51}, thickness = 0.5));
  connect(accelerationRollRateControlVel.bus, navigation.bus) annotation(
    Line(points = {{56, -90}, {80, -90}, {80, -60}, {60, -60}}, color = {255, 204, 51}, thickness = 0.5));
  connect(navigation.bus, constantFlightPathGuidanceDiscrete.bus) annotation(
    Line(points = {{60, -60}, {-58, -60}, {-58, -88}, {-34, -88}}, color = {255, 204, 51}, thickness = 0.5));
  connect(constantFlightPathGuidanceDiscrete.acc_err_int, accelerationRollRateControlVel.acc_err_int) annotation(
    Line(points = {{-12, -88}, {34, -88}, {34, -84}}, color = {0, 0, 127}, thickness = 0.5));
  annotation(
      Icon(coordinateSystem(grid = {2, 0})),
      experiment(StartTime = 0, StopTime = 60, Tolerance = 1e-6, Interval = 0.01));
  end LynxLQVelSimlpeParachute;
